...
CREATE VIEW plays_expanded AS
	SELECT p.*,coalesce(m.author,s.artist) as author,coalesce(m.title,s.song) as title,m.format,m.duration,coalesce(u.nickname,u.tt_nickname) as nickname,u.uid,u.level,(SELECT array_to_string(array_agg(g.user_id),' ') FROM grabs g WHERE g.play_id = p.play_id) as snaggers
	FROM plays p
	LEFT JOIN users u USING (user_id)
